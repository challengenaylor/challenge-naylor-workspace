...
function extractTable(html) {
  ...
  const missingRequired = required.filter((k) => !found.has(k));
  if (missingRequired.length) {
    return {
      status: 'TABLE_STRUCTURE_CHANGED',
      reason: `Could not confidently map required column(s): ${missingRequired.join(', ')}. `
        + `Header cells seen: ${unmapped.map((u) => `"${u.text}"`).join(', ') || '(none unmapped)'}. Refusing to guess a fixed index.`,
    };
  }
